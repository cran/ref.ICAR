# Integrand over tau for the fractional integrated likelihood for ICAR model
# @description Requires neighborhood matrix H. Adaptive quadrature will be used to integrate over the spatial dependence parameter.
...
#
# @return Fractional integrated likelihood for the corresponding ICAR model.
# @export

ref.integrand.spatial.b <- function(tauc,b=0.1,Y,X,H,H.spectral=NULL,Sig_phi=NULL,logmax) {
  ...
  for(i in 1:length(tauc)){
    ...
    integrand <- norm.constant - (0.5*b)*omega.log.det -
      (0.5*aux) - log(tauc[i]) +
